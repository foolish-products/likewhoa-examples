...
// Disable the EL supply in a consistent way to ensure 
// more consistent switching/measurements
void disableELSupply_withResistor(){ 
 // listen to resistor
...
bool prev = false;
bool curr = true;
void disableELSupply_withoutResistor() { 
    PORTF = PORTF | B00100000;
    delayMicroseconds(100);
...
// Set ADC initialization bits - make sure things haven't gotten misconfigured.  
    ...
    disableELSupply_withoutResistor();
...
int impulseThresholdBars[] = {3, 3, 6, 9, 13};
bool isPastThreshold;
#define glowTime_us 1300

void loop() {

    senseResults = senseAll(1700, true);
...
//      }
      ...
      int endInd = startInd + 5 > senseSize - 1 ? senseSize - 1 : startInd + 5;
      
      for (int i = startInd + 1; i < endInd; i++) {
        int normError = senseHistoryTemp[i] - prevMeasure;
        if (normError < 0) { 
          normError = prevMeasure - senseHistoryTemp[i] + 2;
        }
//        long currSquaredError = (prevMeasure - senseHistoryTemp[i]) * (prevMeasure - senseHistoryTemp[i]);
//        if ( prevMeasure < senseHistoryTemp[i]) { 
//          
//        }
//        long currStep = senseHistoryTemp[i + 1] - senseHistoryTemp[i-1];
//        long score = (- currSquaredError * currSquaredError); 
//        
        if (normError < bestSquaredError) { 
          bestSquaredError = normError;
...
//
      ...
      if (senseLag[channel] >= 3) { 
        senseIndex[channel][lagIter] = senseIndex[channel][(lagIter + 1) % lagWindowSize] + 3;
      }

      if (senseLag[channel] <= -2) { 
        senseIndex[channel][lagIter] = senseIndex[channel][(lagIter + 1) % lagWindowSize] - 2;
      }
//      else if (senseLag[channel] <= -3) { 
//        senseIndex[channel][lagIter] = senseIndex[channel][(lagIter + 1) % lagWindowSize] - 3;
//      }
//
//      startInd = senseIndex[channel][lagIter] + 4;//  > 10 ? senseIndex[channel][lagIter] + 2 : 10;
//      for (int i = startInd; i > 2; i--) {   
//        if (senseHistoryTemp[i + 1] - senseHistoryTemp[i - 1] > 11) { 
//          senseIndex[channel][lagIter] = i + 1;
//    //      Serial.println(channel);
//          break;
//        }
//      }
 //     senseIndex[channel][lagIter] = 8;
...
//    
    ...
    for (int channel = 0; channel < channelCount; channel++) { 
      chanImpulse =  smoothedSenseHistory[channel][(smoothedSenseIter + 1 + smoothedSenseSize) % smoothedSenseSize] 
      ...
      else if (chanChangeCount > pureIncreaseThreshold - 5) { 
      ...
      if (switchedCount == 0 && isSense_active[channel] && isPastThreshold){
        isTouched = true;
...
//        extrabufflen += sprintf(extraBuffer + extrabufflen, "%d, ", isSense_active[channel]);
  ...
  Serial.println(logBuffer);
    ...
    if (isTouched && switchedCount == 0) { 
      ...
      switchedCount = senseSize + smoothedSenseSize + lagWindowSize;
    }
