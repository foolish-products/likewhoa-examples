...
#define smoothedTouchWindow 5
#define triggerThreshold 6
...
void loop() {
    senseResults = senseAll(1000, false);
    ...
    for (int channel = 0; channel < channelCount; channel++) { 
      ...
      smoothedSenseHistory[channel][smoothedSenseIter] = getMedian(senseHistoryTemp, senseSize);
    ...
    // Decide if channel is touched
    // Compare the median of the recent (smoothed) history to the latest median.
    // ---------------------------------------------------------------------
    // Trigger a switch when difference exceeds triggerThreshold.  
    // *******
    // smoothedSenseSize determines how far back to look.  
    //   --- If this is large, can get thrown off by ambient electrical signals
    // *******
    // smoothedTouchWindow seperates the median of the past from the current measurement.
    //   --- The capactive measurement changes gradually. Leaving room helps to 
    //       differentiate the trigger!
    Serial.print("Switch: ");
    for (int channel = 0; channel < channelCount; channel++) { 
      smoothedSampleMedian = 0; 
      smoothedSampleMin = 1024;
      for (int i = 1; i < smoothedSenseSize - smoothedTouchWindow + 1; i++) {
        int sample = (i + smoothedSenseIter) % smoothedSenseSize;
        smoothedSampleTemp[i - 1] = smoothedSenseHistory[channel][sample];
        if (smoothedSenseHistory[channel][sample] < smoothedSampleMin) { 
          smoothedSampleMin = smoothedSenseHistory[channel][sample];
        }
      }
      
      smoothedSampleMedian = getMedian(smoothedSampleTemp, smoothedSenseSize - smoothedTouchWindow);

      int sampleDiff = (smoothedSampleMedian - smoothedSampleMin);
      if (sampleDiff < 2.0) {
        sampleDiff = 2.0;
      }
      ...
      if (switchedCount == 0 && 
          (smoothedSampleMedian - smoothedSenseHistory[channel][smoothedSenseIter]) > triggerThreshold) {
        isTouched = true;
        whereTouched[channel] = true;
      }
      
      Serial.print(", ");
      Serial.print((smoothedSampleMedian - smoothedSenseHistory[channel][smoothedSenseIter]));
      Serial.print(", ");
